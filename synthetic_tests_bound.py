# -*- coding: utf-8 -*-
"""
Created on Thu Jul 11 20:21:53 2024
...
y = np.random.normal(size = (nobs_train)).round(2)

y_supp = np.arange(-5, 5, 0.01).round(2)
#%%
# x1 = np.random.normal(loc = 0, scale = 0.1, size = nobs_train).round(2)
x1 = np.random.uniform(high = 1, low = -3, size = nobs_train).round(2)
x2 = np.random.normal(loc = 0.5, scale = 1, size = nobs_train).round(2)

p1 = wemp_to_support(1/(len(x1))*np.ones(len(x1)), x1, y_supp).reshape(-1)
p2 = wemp_to_support(1/(len(x2))*np.ones(len(x2)), x2, y_supp).reshape(-1)

#%% Sanity check 
print(np.quantile(x1, quant))

z_test = solve_opt_prob(x1, np.ones(len(x1)), 'newsvendor', risk_aversion = 0, crit_quant = quant)
print(z_test)

z_test_2 = solve_opt_prob(y_supp, p1.reshape(-1), 'newsvendor', risk_aversion = 0, crit_quant = quant)
print(z_test_2)
#%%
w = np.random.uniform()
...
z_opt = np.quantile(y, quant)
z1 = np.quantile(x1, quant)
z2 = np.quantile(x2, quant)
z_lp = np.quantile(p_comb, quant)

z_ave = (z1+z2)/2

Regret = pd.DataFrame(data = [], columns = ['z1', 'z2', 'z_comb', 'z_ave', 'z_opt'])
z_lp_list = []
for w in np.arange(0, 1, .1):
    temp_regret = pd.DataFrame(data = [], columns = ['z1', 'z2', 'z_comb', 'z_ave', 'z_opt'])
    ...
    z_lp = solve_opt_prob(y_supp, p_comb, 'newsvendor', risk_aversion = 0, crit_quant = quant)    
    z_lp_list.append(z_lp)
    
    z_ave = w*z1+(1-w)*z2

    temp_regret['z1'] = [task_loss(z1, y, problem = 'newsvendor', crit_quant = quant)]
    temp_regret['z2'] = [task_loss(z2, y, problem = 'newsvendor', crit_quant = quant)]
    temp_regret['z_comb'] = [task_loss(z_lp, y, problem = 'newsvendor', crit_quant = quant)]
    temp_regret['z_ave'] = [task_loss(z_ave, y, problem = 'newsvendor', crit_quant = quant)]
    temp_regret['z_opt'] = [task_loss(z_opt, y, problem = 'newsvendor', crit_quant = quant)]
    
    print(z_ave - z_lp)
    
    Regret = pd.concat([Regret, temp_regret])

plt.scatter(10*[1], z_lp_list)
plt.scatter([1], z1, marker = 's')
plt.scatter([1], z2, marker = '^')
plt.show()
...
fig, ax = plt.subplots()
Regret.plot(ax = ax)
plt.xticks(10*np.arange(0,1,.1),  np.arange(0,1,.1).round(1))
