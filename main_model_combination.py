...
Decision-Focused Forecast Combination/ main script/ second experimental setup: different probabilistic forecasting models for the same location
...
results_path = f'{cd}\\results\\different_prob_models'
data_path = f'{cd}\\data'
...
#%%
    ...
    for i, learner in enumerate(all_learners):
        if learner == 'clim':
            ...
            temp_p = wemp_to_support(train_w_dict[learner][0:1], trainY[target_zone].values, y_supp).repeat(len(train_w_dict[learner]),0)
            train_p_list.append(temp_p)
            
            temp_p = wemp_to_support(test_w_dict[learner][0:1], trainY[target_zone].values, y_supp).repeat(len(test_w_dict[learner]),0)
