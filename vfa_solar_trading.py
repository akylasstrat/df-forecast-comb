# -*- coding: utf-8 -*-
"""
Created on Mon Aug 12 14:19:33 2024
...
def task_loss(pred, actual, problem, return_mean = True, **kwargs):
    ...
    if problem == 'mse':
        if return_mean:
            ...
            return np.square(actual_copy-pred_copy).reshape(-1,1)
        ...
        if return_mean:
            ...
            return np.maximum(kwargs['crit_quant']*(actual_copy - pred_copy), (kwargs['crit_quant']-1)*(actual_copy - pred_copy)).reshape(-1,1)
        ...
        if return_mean:
            ...
            return pwl_loss.reshape(-1,1) + kwargs['risk_aversion']*square_loss.reshape(-1,1)
        ...
        if return_mean:
            ...
            return (1- kwargs['risk_aversion'])*pinball_loss.reshape(-1,1) + kwargs['risk_aversion']*square_loss.reshape(-1,1)
...
for tup in tuple_list[row_counter:]:
    ...
    train_VFA = np.zeros((n_train_obs, len(train_p_list)))
    test_VFA = np.zeros((n_test_obs, len(test_p_list)))
    
    print('Finding value functions in historical data')
    for j in range(N_experts):        
        train_VFA[:,j] = task_loss(trainZopt[:,j], train_targetY, target_problem, crit_quant = critical_fractile, risk_aversion = risk_aversion, 
                                       return_mean=False).reshape(-1)
        
        test_VFA[:,j] = task_loss(testZopt[:,j], testY.values, target_problem, crit_quant = critical_fractile, risk_aversion = risk_aversion, 
                                       return_mean=False).reshape(-1)
    
    
    train_VFA = train_VFA.T.reshape(-1,1)
    train_X_VFA = np.concatenate(train_p_list)
    test_X_VFA = np.concatenate(test_p_list)

    # train_X_VFA = np.concatenate([train_X_VFA, np.square(train_X_VFA)], axis=1)
    # Feature data set: probabilities at each location
    
    #%%
    from sklearn.linear_model import LinearRegression, Ridge, Lasso
    
    
    lr_vfa_model = Ridge().fit(train_X_VFA, train_VFA)
    fitted_vfa = lr_vfa_model.predict(train_X_VFA).reshape(-1,1)
    error_aprx = train_VFA - fitted_vfa
    
    print(np.square(error_aprx).mean())
    #%%
    from torch_layers_functions import *
    
    tensor_train_VFA = torch.FloatTensor(train_VFA[:-3000])
    tensor_valid_VFA = torch.FloatTensor(train_VFA[-3000:])
    
    tensor_trainX_VFA = torch.FloatTensor(train_X_VFA[:-3000])
    tensor_validX_VFA = torch.FloatTensor(train_X_VFA[-3000:])
    tensor_testX_VFA = torch.FloatTensor(test_X_VFA)

    train_data_loader = create_data_loader([tensor_trainX_VFA, tensor_train_VFA], batch_size = 512, shuffle= True)    
    valid_data_loader = create_data_loader([tensor_validX_VFA, tensor_valid_VFA], batch_size = 512, shuffle= True)    
    ...
    #%%
    plt.plot(vfa_model.model[0].weight.detach().numpy().T)
    plt.show()
    
    print(np.square(vfa_model.predict(tensor_trainX_VFA)).mean())
    
    plt.plot(train_VFA[:1000])
    # plt.plot(fitted_vfa[:1000])
    plt.plot(vfa_model.predict(tensor_trainX_VFA)[:1000])
    plt.show()
    
    #%% Decision-focused linear pooling with Value-function approximation 
    from torch_layers_functions import *
    ...
    lpool_vfa_newsv_model = LinearPool_VFA_Layer(vfa_model, num_inputs=N_experts, support = torch.FloatTensor(y_supp),
                                                gamma = 0, problem = target_problem, critic_fract = critical_fractile, risk_aversion = risk_aversion, 
                                                projection_simplex = True)
    
    optimizer = torch.optim.Adam(lpool_vfa_newsv_model.parameters(), lr = 1e-2)
    
    lpool_vfa_newsv_model.train_model(train_data_loader, valid_data_loader, optimizer, epochs = num_epochs, 
                                      patience = patience, validation = True, relative_tolerance = 1e-5)
